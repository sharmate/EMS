...
//include('session.php');
include 'config.php';
$columns = array('name', 'email', 'outing_option', 'preferred_location', 'confirmation_status');

$query = "SELECT aos.*, employee.name, employee.email FROM annual_outing_suggestions as aos, employee";
...
while($row = mysqli_fetch_array($result))
{
    ...
    $sub_array[] = '<div class="update" data-id="'.$row["employee_id"].'" data-column="contact">' . $row["days_option"] . '</div>';
    $sub_array[] = '<div class="update" data-id="'.$row["employee_id"].'" data-column="password">' . $row["preferred_location"] . '</div>';
    $sub_array[] = '<div class="update" data-id="'.$row["employee_id"].'" data-column="password">' . $row["preferred_month"] . '</div>';
    
    $sub_array[] = '<div class="update" data-id="'.$row["employee_id"].'" data-column="password">' . $row["confirmation_status"] . '</div>';

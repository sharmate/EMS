...
//include('session.php');
include 'config.php';
$columns = array('name', 'contact', 'email', 'laptop_allowcated', 'model_id');

$query = "SELECT aos.*, employee.name, employee.contact, employee.email FROM laptop_verification as aos, employee";
...
while($row = mysqli_fetch_array($result))
{
    ...
    $sub_array[] = '<div class="update" data-id="'.$row["employee_id"].'" data-column="laptop_allowcated">' . $row["laptop_allowcated"] . '</div>';
    $sub_array[] = '<div class="update" data-id="'.$row["employee_id"].'" data-column="model_id">' . $row["model_id"] . '</div>';
    
    $tmpAction = '<button type="button" name="view" class="btn  btn-success btn-xs view-attendance-btn" value="View" onclick="viewLaptopVerification('.$row['employee_id'].')" title="View"><i class="fa fa-eye"></i></button>';
